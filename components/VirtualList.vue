  ...
  export default Vue.extend({
    ...
    methods: {
      getItemHeight(index: number): number {
        ...
        return (this.heightCache as Map<string | number, number>).get(key) ?? this.itemHeight;
      },
      ...
      measureRows(): void {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          const oldHeight = (this.heightCache as Map<string | number, number>).get(key) ?? this.itemHeight;
          if (measured > 0 && oldHeight !== measured) {
            (this.heightCache as Map<string | number, number>).set(key, measured);
            changed = true;
